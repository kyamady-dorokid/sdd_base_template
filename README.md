...
すでに `CLAUDE.md` などがあるリポジトリに `init` すると、**勝手には進まず扱いを確認**します。

**オプションを付けない場合（推奨・端末での対話）** — 扱いを対話で質問します:

```bash
npx -y github:kyamady-dorokid/sdd_base_template init
# → [o] overwrite（対比して上書き・既定） / [k] keep（温存） / [c] compare（差分のみ） を質問
```

**扱いを明示する場合（自動化・CI向け）** — 質問なしで実行します:
...
> 非対話（CI等）でオプション未指定の場合は、安全のため**停止して明示を促します**（黙って進みません）。
...
- 実行結果は毎回 `.kiro/sdd-base-update-report.md` に出力されます（実行のたびに上書きされるため `.gitignore` 対象）。自動コミットは行いません。
...
- 本リポジトリ: **MIT License, © 2026 kyamady-dorokid**（全文は [`LICENSE`](LICENSE)）。
- 本リポジトリへの貢献（Pull Request 等）は、特段の合意がない限り本リポジトリと同じ **MIT ライセンス**の下で提供されたものとみなします（inbound=outbound）。
