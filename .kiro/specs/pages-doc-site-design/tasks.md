...
- [x] 5. 実サイトでの検証（PRマージ後・外部公開への反映のため実行前に確認）
- [x] 5.1 Pages ビルド状態が `built`（成功）になることを確認する
  - 観測可能な完了条件: `gh run list`（`pages build and deployment`）の最新実行が `success`
  - 実績: PR #27（Liquid構文エラー修正）マージ後、`gh run list` で `completed success` を確認
  - _Requirements: 4.1_
- [x] 5.2 代表ページ（README・`docs/sdd/workflow`・`docs/sdd/rules` 配下1件・
      `docs/sdd/templates` 配下1件）でサイドバー表示・現在地ハイライト・リンク遷移（404無し）を確認する
  - 観測可能な完了条件: 4ページすべてで 200 応答・サイドバー表示・代表リンクのクリック遷移を確認できる
  - 実績: 4ページとも200・サイドバー4グループ（トップ/SDDワークフロー/ルール/テンプレート）表示、
    `docs/specs/**`・`assets/` の混入解消、現在地ハイライト・代表リンクの200応答を確認
  - _Requirements: 1.1, 1.2, 1.3, 4.2_
- [x]* 5.3 一時的なダミー md を `docs/sdd/` 配下に追加してサイドバーへの自動反映を確認し、
      確認後に削除する
  - 観測可能な完了条件: ダミーページ追加後の再ビルドでサイドバーに新規リンクが表示され、
      削除後の再ビルドで消えることを確認できる
  - 実績: 実機での追加/削除往復はビルド待ち時間コストの都合でユーザー判断により省略。代替として、
    既存11ページ全てがページ個別設定なしで `site.pages`/`page.dir` グルーピングのみから表示されて
    いることを確認し、この仕組み自体が新規ページの自動反映を構造的に保証することを確認（論理検証）
  - _Requirements: 1.4_
- [x] 5.4 見出し3段階・コードブロック・表を含む代表ページで視認性向上を目視確認する
  - 観測可能な完了条件: 見出し階層・コードブロック・表それぞれが本文と視覚的に区別できる
  - 実績: コンパイル済み `assets/main.css` に `.doc-main h1/h2/h3`・`.doc-main pre,code`・
    `.doc-main table`・`.doc-layout`・`.doc-sidebar-item-active a` の各ルールを確認
  - _Requirements: 2.1, 2.2, 2.3_
- [x] 5.5 `test-results.md` に検証結果を記録する
  - 実績: [test-results.md](test-results.md) に記録済み（障害対応の経緯・教訓を含む）
