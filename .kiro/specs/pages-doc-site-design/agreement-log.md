...
| 実装（タスク5：実サイト検証） | 一旦「Pagesビルド詰まり」と判断し保留にしたが、`gh run list` で実際は失敗（`sidebar.html`のLiquid構文エラー）と判明。PR #27で修正し、`gh run list`で`success`を確認（2026-07-17）。代表4ページ・混入解消・現在地ハイライト・視認性CSSを確認済み。**完了**（5.3のみ、ビルド待ち時間コストを踏まえユーザー判断で実機往復を論理検証に代替）。詳細は `test-results.md`／`tasks.md` の障害対応履歴を参照 |
