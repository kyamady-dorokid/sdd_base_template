...
├── run.sh                          # unit/ → integration/ の順に実行
├── lib/assert.sh
├── unit/                           # 外部副作用を持たない純粋ロジックのテスト
│   └── test_{hash,lock,merge}.sh
└── integration/                    # sync.sh を実際に起動するエンドツーエンドのテスト
    └── test_sync_{init,apply,report}.sh

<対象リポジトリ側（sync実行後に生成・コミットされる）>
.kiro/
├── sdd-base.lock                  # 新規: lock ファイル（行指向）
├── sdd-base-snapshot/             # 新規: 適用時点の overlay 相当の複製（3-wayマージのbase）
└── sdd-base-update-report.md      # 新規: 実行結果レポート（sync実行のたびに上書き。.gitignore 対象・コミットしない）
```
...
>
> **`.kiro/specs/<id>/` 配下に置かない理由**: `.kiro/specs/<id>/` は
> `docs/sdd/workflow.md`「タスクディレクトリ構成」が定めるプロセス記録専用の場所
> （`spec.json`/`requirements.md`/`design.md`/`agreement-log.md` 等）であり、実行コードの置き場としては
> 想定されていない。`payload/scripts/sync_lib/*.sh` は特定タスクに紐づかない永続的なプロダクトコードで
> あるため、そのテストも特定タスクの spec フォルダではなく、コードと対になる安定した場所
> （`tests/unit/` `tests/integration/`）に置く（将来別タスクがこのコードを改修する際にテストの所在が
> 迷子にならないようにするため）。この配置規約は `CLAUDE.md`/`AGENTS.md` に恒久ルールとして明文化する。
