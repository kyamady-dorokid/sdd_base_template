...
- **承認後に承認済み要件・設計から外れる必要が生じたら、勝手に決めず人間の確認に戻る。**
  ...
  再承認後は、決定を requirements/design に反映し、`spec.json` の該当ゲートを更新してから実装を再開する。
  **agreement-log への事後記録だけで押し通してはならない**（記録のみで済ませてよいのは、設計に影響しない
  些末な実装詳細に限る）。判断基準: 「これは人間が承認した設計の範囲内か？ 範囲外なら止めて戻る」。
